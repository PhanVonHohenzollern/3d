// Port of the QMatrix4x4 operations used by renderer/Viewport3D:
// setToIdentity, operator*, operator*(QVector4D), perspective, lookAt,
// translate, inverted and normalMatrix.
//
// Storage is column-major exactly like QMatrix4x4::m[col][row] / constData():
// element (row, col) lives at data[col * 4 + row], so `data` can be passed to
// uniformMatrix4fv(..., false, data) just as QOpenGLShaderProgram does.
// Like QMatrix4x4 the elements are floats (Float32Array); inverted() and
// normalMatrix() compute in double precision, as Qt does.

import { QVector3D, QVector4D, qFuzzyIsNull } from './Vector3D';

export class QMatrix4x4 {
  readonly data = new Float32Array(16);

  /** Identity, like QMatrix4x4(). */
  ...
  /** m[col][row] */
  private m(col: number, row: number): number { return this.data[col * 4 + row]; }

  /** QMatrix4x4::operator()(row, column) */
  get(row: number, column: number): number { return this.data[column * 4 + row]; }
  ...
  clone(): QMatrix4x4 { return new QMatrix4x4(this.data); }
    ...
    for (let col = 0; col < 4; ++col) {
      for (let row = 0; row < 4; ++row) {
        result.data[col * 4 + row] =
          m1.m(0, row) * m2.m(col, 0) + m1.m(1, row) * m2.m(col, 1)
          + m1.m(2, row) * m2.m(col, 2) + m1.m(3, row) * m2.m(col, 3);
      }
    }
    return result;
  }
  ...
  /** this * other */
  times(other: QMatrix4x4): QMatrix4x4 { return QMatrix4x4.multiply(this, other); }
  ...
  /** QMatrix4x4::perspective(verticalAngle, aspectRatio, nearPlane, farPlane) */
  perspective(verticalAngle: number, aspectRatio: number, nearPlane: number, farPlane: number): void {
    ...
    const set = (col: number, row: number, value: number) => { p.data[col * 4 + row] = value; };
  ...
  lookAt(eye: QVector3D, center: QVector3D, up: QVector3D): void {
    ...
    const set = (col: number, row: number, value: number) => { r.data[col * 4 + row] = value; };
    set(0, 0, side.x);
    set(1, 0, side.y);
    set(2, 0, side.z);
    set(3, 0, 0);
  ...
  /** QMatrix4x4::translate(vector): *this = *this * T(vector) */
  ...
  /** QMatrix4x4::inverted(bool *invertible), general (double precision) path. */
  inverted(): { matrix: QMatrix4x4; invertible: boolean } {
    const mm = (col: number, row: number) => this.m(col, row);
    const det2 = (c0: number, c1: number, r0: number, r1: number) =>
      mm(c0, r0) * mm(c1, r1) - mm(c0, r1) * mm(c1, r0);
    const det3 = (c0: number, c1: number, c2: number, r0: number, r1: number, r2: number) =>
      mm(c0, r0) * det2(c1, c2, r1, r2)
      - mm(c1, r0) * det2(c0, c2, r1, r2)
      + mm(c2, r0) * det2(c0, c1, r1, r2);
    ...
    const set = (col: number, row: number, value: number) => { inv.data[col * 4 + row] = value; };
  ...
  /**
   * QMatrix4x4::normalMatrix(): the inverse transpose of the upper-left 3x3,
   * as a column-major QMatrix3x3 (9 floats) for uniformMatrix3fv. Returns the
   * identity when that block is singular, as Qt does.
   */
  normalMatrix(): Float32Array {
    const inv = new Float32Array([1, 0, 0, 0, 1, 0, 0, 0, 1]);
    const mm = (col: number, row: number) => this.m(col, row);
    let det = mm(0, 0) * (mm(1, 1) * mm(2, 2) - mm(1, 2) * mm(2, 1))
