// Port of geometry/ConnectorPreview.{h,cpp}.
//
// Like the C++, connector tests reuse the engine's API adapters and mesh
// builders through the public PreviewGeometryEngine.build(): a synthesized
// makeVerySimpleTube / makeBox RuntimeApiCall is run through a private
// RuntimeResult, so no internal builder has to be exported. Nothing here is
// added to the user's RuntimeResult or API Trace.

import { CppException, what } from '../runtime/CppCompat';
...
/**
 * Preview/Link signs are reversed relative to SDK component signs.
 * Both axis labels and connector tests use this mapping.
 */
...
/** Link tests stay outside RuntimeResult/API Trace and never modify source geometry. */
...
export function buildConnectorPreview(definition: ConnectorDefinition, evaluate: ConnectorExpressionEvaluator): ConnectorPreview {
  ...
  result.length = (width < height ? height : width) * 0.35; // std::max(width, height)
  const end = result.point.add(result.direction.mul(result.length));

  // Reuse the same API adapters as source geometry instead of a second set of primitives.
