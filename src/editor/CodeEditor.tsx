...
import {
  Decoration, drawSelection, EditorView, GutterMarker, gutterLineClass, highlightActiveLine, keymap, lineNumbers,
  type Command, type DecorationSet, type ViewUpdate,
...
const traceLinesField = StateField.define<TraceLines>({
  ...
  provide: (field) => [
    ...
    EditorView.decorations.compute([field, 'doc'], (state): DecorationSet => {
      ...
      return Decoration.set(validTraceLines(state, trace).map((line) =>
        (line === trace.activeLine ? activeTraceLineDecoration : traceLineDecoration).range(state.doc.line(line).from)));
    }),
    // CodeEditor::lineNumberAreaPaintEvent(): traced numbers are bold on amber.
    gutterLineClass.compute([field, 'doc'], (state) => {
      const trace = state.field(field);
      return RangeSet.of(validTraceLines(state, trace).map((line) =>
        tracedLineNumberMarker.range(state.doc.line(line).from)));
    }),
  ],
});
...
const syntaxColors = HighlightStyle.define([
  { tag: [tags.keyword, tags.definitionKeyword, tags.modifier, tags.operatorKeyword, tags.self, tags.null, tags.bool], color: '#569cd6' },
  { tag: tags.controlKeyword, color: '#c586c0' },
  { tag: [tags.typeName, tags.standard(tags.typeName), tags.namespace], color: '#4ec9b0' },
  { tag: [tags.function(tags.variableName), tags.function(tags.propertyName), tags.function(tags.definition(tags.variableName))], color: '#dcdcaa' },
...
const editorTheme = EditorView.theme({
  ...
  '&.cm-focused > .cm-scroller > .cm-selectionLayer .cm-selectionBackground, .cm-selectionBackground': { backgroundColor: '#264f78' },
  ...
  useImperativeHandle(ref, (): CodeEditorHandle => {
    ...
    return {
      toPlainText() {
        ...
        return view().state.doc.toString().replace(/\u00a0/g, ' ').replace(/[\u2028\u2029]/g, '\n');
      },
      clear() {
        const v = view();
