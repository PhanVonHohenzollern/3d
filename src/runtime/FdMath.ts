// Port of runtime/FdMath.h: the SDK point/vector math.
//
// The C++ types have value semantics. These classes are immutable so they can
// be shared freely; C++ mutating members (normalize, rotateBy, mirror, +=)
// return a new instance and the interpreter writes it back to the l-value.
// Floating-point operations keep the C++ evaluation order.
...
export class FdVector3d {
  ...
  /** Result of C++ `v.normalize()` (which mutates v and returns *this). */
  normalize(): FdVector3d {
    const len = this.length();
    if (len === 0) return this;
    return new FdVector3d(this.x / len, this.y / len, this.z / len);
  }
  /** Result of C++ `v.rotateBy(angle, axis)`. */
...
export class FdPoint3d {
  ...
  /** Result of C++ `p.rotateBy(angle, axis[, wrtPoint])`. */
