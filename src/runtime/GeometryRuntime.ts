...
import {
  CppException, doubleToInt64, isalnum, isalpha, isdigit, isspace, stod, stoll, strtod, wrapInt64,
} from './CppCompat';
import { FdPoint3d, FdVector3d } from './FdMath';
import {
  emptyApiCall,
  type RuntimeApiCall, type RuntimeArgumentTrace, type RuntimeDiagnostic, type RuntimeParameterRequest,
  type RuntimeResult, type RuntimeValueSource, type RuntimeVariableChange,
} from './RuntimeTypes';
import {
  RuntimeArray, isArray, isBool, isDouble, isInt, isPoint, isString, isVector,
  runtimeCoerceToType, runtimeDeepCopy, runtimeDefaultValueForType, runtimeInteger, runtimeNumber,
  runtimeTruthy, runtimeTypeName, runtimeValueToCompactString, type RuntimeValue,
} from './RuntimeValue';
...
function numericRunEnd(source: string, start: number): number {
  ...
  while (p < source.length) {
    const ch = source[p];
    if (isalnum(ch) || ch === '.') { ++p; continue; }
    if ((ch === '+' || ch === '-') && p > start && 'eEpP'.includes(source[p - 1])) { ++p; continue; }
    break;
  }
  return p;
}
...
class Lexer {
  ...
  scan(): Token[] {
    ...
    else if (this.m_pos === 0 && src.length >= 3
      && src.charCodeAt(0) === 0xef && src.charCodeAt(1) === 0xbb && src.charCodeAt(2) === 0xbf) this.m_pos = 3;
    let lineStart = true;
    while (this.m_pos < src.length) {
      const c = src[this.m_pos];
      if (c === '\n') {
        ++this.m_line; ++this.m_pos; lineStart = true; continue;
      }
      if (isspace(c)) { ++this.m_pos; continue; }
      ...
      if (isdigit(c) || (c === '.' && isdigit(this.peek(1)))) {
        ...
        while (this.m_pos < src.length) {
          ...
          if (suffix === 'u' || suffix === 'U' || suffix === 'l' || suffix === 'L'
            || (!hex && (suffix === 'f' || suffix === 'F'))) ++this.m_pos;
          else break;
        }
...
function tokensToExpression(tokens: readonly Token[]): string {
  ...
  for (const token of tokens) {
    ...
    const previousWord = previousKind === TokKind.Identifier || previousKind === TokKind.Number || previousKind === TokKind.String;
...
function isScalarTypeToken(token: Token): boolean {
  ...
  return t === 'double' || t === 'float' || t === 'int' || t === 'short'
    || t === 'long' || t === 'bool' || t === 'char';
}

function parseRuntimeType(tokens: readonly Token[], pos: Ref<number>, type: Ref<string>): boolean {
  const original = pos.value;
  const qualifiers = () => {
    while (pos.value < tokens.length && (isIdentifier(tokens[pos.value], 'const') || isIdentifier(tokens[pos.value], 'volatile'))) ++pos.value;
  };
  qualifiers();
  if (pos.value >= tokens.length || tokens[pos.value].kind !== TokKind.Identifier) { pos.value = original; return false; }
  type.value = tokens[pos.value++].text;
  while (pos.value + 1 < tokens.length && isSymbol(tokens[pos.value], '::') && tokens[pos.value + 1].kind === TokKind.Identifier) {
    type.value += '::' + tokens[pos.value + 1].text; pos.value += 2;
  }
  if (type.value !== 'FdPoint3d' && type.value !== 'FdVector3d' && !sdkTypeDefinition(type.value)
    && !isScalarTypeToken(makeToken(TokKind.Identifier, type.value))) { pos.value = original; return false; }
  qualifiers();
  if (type.value === 'char' && pos.value < tokens.length && isSymbol(tokens[pos.value], '*')) {
    type.value = 'char*'; ++pos.value; qualifiers();
  }
  return true;
}
...
function isNumericType(name: string): boolean {
  ...
  return type === 'double' || type === 'float' || type === 'int' || type === 'short'
    || type === 'long' || type === 'bool';
}
...
function simpleInitializerValue(tokens: readonly Token[], type: string): string {
  ...
  if (tokens.length === 2 && (isSymbol(tokens[0], '-') || isSymbol(tokens[0], '+')) && tokens[1].kind === TokKind.Number)
  ...
  for (let i = 0; i + 1 < tokens.length; ++i) {
    ...
    for (const fragment of fragments) {
      ...
      for (let k = 0; k < fragment.length; ++k) {
        ...
        if (word === 'const' || word === 'volatile' || isScalarTypeToken(fragment[k]))
          continue;
        namePos = k;
        break;
      }
...
function scanGetValParameters(code: string): RuntimeParameterRequest[] {
  ...
  for (let i = 0; i + 1 < tokens.length; ++i) {
    ...
    for (const t of args[1]) {
      if (t.kind === TokKind.Identifier) { baseVariable = t.text; break; }
    }
...
class ProgramParser {
  ...
  parse(): Statement {
    ...
    root.startLine = 1;
    while (!this.atEnd()) {
      if (isSymbol(this.current(), '}')) { this.advance(); continue; }
  ...
  private looksLikeFunctionDefinition(): boolean {
    if (isIdentifier(this.current(), 'if') || isIdentifier(this.current(), 'for') || isIdentifier(this.current(), 'else')) return false;
    ...
    for (let p = this.m_pos; p < this.m_tokens.length; ++p) {
      ...
      if (isSymbol(t, '(')) { ++paren; sawParen = true; }
      else if (isSymbol(t, ')')) --paren;
      else if (isSymbol(t, '[')) ++bracket;
      else if (isSymbol(t, ']')) --bracket;
  ...
  private parseFunction(): Statement {
    ...
    for (let i = lparen; i > start; --i) {
      if (this.m_tokens[i - 1].kind === TokKind.Identifier) { fn.functionName = this.m_tokens[i - 1].text; break; }
    }
...
function tokensToText(tokens: readonly Token[]): string {
  ...
  for (let i = 0; i < tokens.length; ++i) {
    ...
    if (i + 1 < tokens.length) {
      const n = tokens[i + 1];
      if ((t.kind === TokKind.Identifier || t.kind === TokKind.Number || t.kind === TokKind.String) &&
        (n.kind === TokKind.Identifier || n.kind === TokKind.Number || n.kind === TokKind.String)) out += ' ';
    }
  }
  return out;
}
...
class ExprParser {
  ...
  constructor(private readonly m_tokens: readonly Token[], private readonly m_runtime: GeometryRuntime) {}
  ...
  private atEnd(): boolean { return this.m_pos >= this.m_tokens.length; }
  private current(offset = 0): Token {
    const p = this.m_pos + offset;
    return p < this.m_tokens.length ? this.m_tokens[p] : kEndToken;
  }
  private match(s: string): boolean {
    if (!this.atEnd() && this.current().text === s) { ++this.m_pos; return true; }
    return false;
  }
  ...
  private parseEquality(): RuntimeValue {
    ...
    while (!this.atEnd() && (this.current().text === '==' || this.current().text === '!=')) {
      const op = this.current().text; ++this.m_pos;
  ...
  private parseRelational(): RuntimeValue {
    ...
    while (!this.atEnd() && (this.current().text === '<' || this.current().text === '>' || this.current().text === '<=' || this.current().text === '>=')) {
      const op = this.current().text; ++this.m_pos;
      const rhs = this.parseAdditive();
      let r = false;
      if (isNumeric(lhs) && isNumeric(rhs)) {
        const a = runtimeNumber(lhs), b = runtimeNumber(rhs);
        if (op === '<') r = a < b;
        else if (op === '>') r = a > b;
        else if (op === '<=') r = a <= b;
        else r = a >= b;
  ...
  private parseMultiplicative(): RuntimeValue {
    ...
    while (!this.atEnd() && (this.current().text === '*' || this.current().text === '/' || this.current().text === '%')) {
      const op = this.current().text; ++this.m_pos;
  ...
  private isCastAhead(): boolean {
    ...
    const type = { value: '' };
    return parseRuntimeType(this.m_tokens, pos, type) && isNumericType(type.value)
      && pos.value < this.m_tokens.length && isSymbol(this.m_tokens[pos.value], ')');
  }
  ...
  private parseUnary(): RuntimeValue {
    ...
    if (!this.atEnd() && isIdentifier(this.current(), 'static_cast')) {
      ++this.m_pos; this.expect('<');
      const type = { value: '' };
      const pos = { value: this.m_pos };
      const parsed = parseRuntimeType(this.m_tokens, pos, type);
      this.m_pos = pos.value;
      if (!parsed || !isNumericType(type.value))
        throw runtimeError('unsupported static_cast type');
      this.expect('>'); this.expect('(');
      const value = this.parseConditional(); this.expect(')');
  ...
  private callFreeFunction(name: string, args: readonly RuntimeValue[]): RuntimeValue {
    ...
    if (name === 'FdPoint3d' || name === 'FdVector3d') {
      ...
      const x = runtimeNumber(args[0]), y = runtimeNumber(args[1]), z = runtimeNumber(args[2]);
      return name === 'FdPoint3d' ? new FdPoint3d(x, y, z) : new FdVector3d(x, y, z);
    }
    if (name === 'sin') { if (args.length !== 1) throw runtimeError('sin requires 1 argument'); return Math.sin(runtimeNumber(args[0])); }
    if (name === 'cos') { if (args.length !== 1) throw runtimeError('cos requires 1 argument'); return Math.cos(runtimeNumber(args[0])); }
    if (name === 'tan') { if (args.length !== 1) throw runtimeError('tan requires 1 argument'); return Math.tan(runtimeNumber(args[0])); }
    if (name === 'asin') { if (args.length !== 1) throw runtimeError('asin requires 1 argument'); return Math.asin(runtimeNumber(args[0])); }
    if (name === 'acos') { if (args.length !== 1) throw runtimeError('acos requires 1 argument'); return Math.acos(runtimeNumber(args[0])); }
    if (name === 'atan') { if (args.length !== 1) throw runtimeError('atan requires 1 argument'); return Math.atan(runtimeNumber(args[0])); }
    if (name === 'atan2') { if (args.length !== 2) throw runtimeError('atan2 requires 2 arguments'); return Math.atan2(runtimeNumber(args[0]), runtimeNumber(args[1])); }
    if (name === 'sqrt') { if (args.length !== 1) throw runtimeError('sqrt requires 1 argument'); return Math.sqrt(runtimeNumber(args[0])); }
    if (name === 'floor') { if (args.length !== 1) throw runtimeError('floor requires 1 argument'); return Math.floor(runtimeNumber(args[0])); }
    if (name === 'ceil') { if (args.length !== 1) throw runtimeError('ceil requires 1 argument'); return Math.ceil(runtimeNumber(args[0])); }
    if (name === 'round') { if (args.length !== 1) throw runtimeError('round requires 1 argument'); return cppRound(runtimeNumber(args[0])); }
    if (name === 'pow') { if (args.length !== 2) throw runtimeError('pow requires 2 arguments'); return cppPow(runtimeNumber(args[0]), runtimeNumber(args[1])); }
    if (name === 'abs' || name === 'fabs') { if (args.length !== 1) throw runtimeError(name + ' requires 1 argument'); return Math.abs(runtimeNumber(args[0])); }
    if (name === 'strcmp') {
  ...
  private callMethod(value: RuntimeValue, method: string, args: readonly RuntimeValue[]): RuntimeValue {
    if (isVector(value)) {
      ...
      if (method === 'length') { if (args.length !== 0) throw runtimeError('length takes no arguments'); return v.length(); }
      if (method === 'lengthSqrd') { if (args.length !== 0) throw runtimeError('lengthSqrd takes no arguments'); return v.lengthSqrd(); }
      if (method === 'normal') {
      ...
      if (method === 'rotateBy') {
        ...
        if (args.length !== 2 || !isVector(axis))
          throw runtimeError('FdVector3d::rotateBy requires angle and axis');
        let copy = v;
      ...
      if (method === 'mirror') {
        const normal = args[0];
        if (args.length !== 1 || !isVector(normal))
          throw runtimeError('mirror requires FdVector3d');
        let copy = v; copy = copy.mirror(normal); return copy;
      }
      if (method === 'angleTo') {
        const other = args[0];
    ...
    if (isPoint(value)) {
      ...
      if (method === 'rotateBy') {
        const axis = args[1];
        if (args.length === 2 && isVector(axis)) {
          let copy = p; copy = copy.rotateBy(runtimeNumber(args[0]), axis); return copy;
        }
        const center = args[2];
        if (args.length === 3 && isVector(axis) && isPoint(center)) {
          let copy = p; copy = copy.rotateBy(runtimeNumber(args[0]), axis, center); return copy;
        }
  ...
  private parsePostfix(value: RuntimeValue): RuntimeValue {
    while (!this.atEnd()) {
      ...
      if (this.match('.')) {
        ...
        const member = this.current().text; ++this.m_pos;
  ...
  private parsePrimary(): RuntimeValue {
    ...
    if (this.current().kind === TokKind.String) {
      const s = this.current().text; ++this.m_pos; return s;
    }
    if (this.match('(')) {
      const v = this.parseConditional();
      this.expect(')');
    ...
    if (this.current().kind !== TokKind.Identifier) throw runtimeError("expected expression near '" + this.current().text + "'");
    ...
    if ((name === 'getExtInsSize' || name === 'getIntInsSize')
      && this.current().text === '(' && this.current(1).kind === TokKind.Identifier && this.current(2).text === ')') {
      ++this.m_pos;
      const destName = this.current().text; ++this.m_pos;
      this.expect(')');
      ...
      } catch (e) { stdException(e); return false; }
    ...
    if (this.match('::')) {
      ...
      while (this.match('::')) {
        ...
        qualified += '::' + this.current().text; ++this.m_pos;
      }
...
class MapSlot implements RuntimeValueSlot {
  constructor(private readonly map: Map<string, RuntimeValue>, private readonly key: string) {}
  get(): RuntimeValue { return this.map.get(this.key); }
  set(value: RuntimeValue): void { this.map.set(this.key, value); }
}

/** An element of a (shared) runtime array. */
class ArraySlot implements RuntimeValueSlot {
  constructor(private readonly array: RuntimeArray, private readonly index: number) {}
  get(): RuntimeValue { return this.array.elements[this.index]; }
  set(value: RuntimeValue): void { this.array.elements[this.index] = value; }
}
...
class RuntimeExecutor {
  ...
  constructor(private readonly m_runtime: GeometryRuntime, private readonly m_maxLine: number) {}
  ...
  private safeExecute(s: Statement, fn: () => void): void {
    try { fn(); }
    catch (e) { this.m_runtime.addDiagnostic(s.startLine, stdException(e).message); }
  }
  ...
  private initializerValue(tokens: readonly Token[], type: string, dims: readonly number[], level: number): RuntimeValue {
  ...
  private inferArrayDimensions(initializer: readonly Token[], dims: number[], level = 0): void {
    if (level >= dims.length || initializer.length < 2
      || !isSymbol(initializer[0], '{') || !isSymbol(initializer[initializer.length - 1], '}')) return;
  ...
  private executeDeclaration(tokens: readonly Token[], line: number, userVariables = true): void {
    ...
    for (const decl of declarators) {
      ...
      if (p >= decl.length || decl[p].kind !== TokKind.Identifier) throw runtimeError('expected variable name in declaration');
      ...
      const initializer = tail.length !== 0 && isSymbol(tail[0], '=')
        ? sliceTokens(tail, 1, tail.length) : tail;
      this.m_runtime.setVariable(
        name, runtimeDeepCopy(value), userVariables, line, 'declare', tokensToExpression(initializer));
    }
  }
  ...
  private resolveLValue(tokens: readonly Token[]): LValueRef {
    ...
    while (p < tokens.length) {
      ...
      if (isSymbol(tokens[p], '.')) {
        ++p;
        if (p >= tokens.length || tokens[p].kind !== TokKind.Identifier) throw runtimeError('expected member name after .');
  ...
  private writeLValue(ref: LValueRef, value: RuntimeValue): void {
    ...
    if (ref.member === '') {
      ...
      if (type === 'double' || type === 'int' || type === 'bool' || type === 'FdPoint3d' || type === 'FdVector3d' || type === 'string')
        ref.value.set(runtimeCoerceToType(value, type === 'string' ? 'string' : type));
      else
        ref.value.set(runtimeDeepCopy(value));
      return;
    }
  ...
  private executeMutatingMethod(tokens: readonly Token[], line: number): boolean {
    ...
    for (let i = 0; i + 2 < tokens.length; ++i) {
      ...
      else if (bracket === 0 && isSymbol(tokens[i], '.') && tokens[i + 1].kind === TokKind.Identifier && isSymbol(tokens[i + 2], '(')) {
        dot = i; break;
      }
    }
    ...
    const sources = this.m_runtime.captureValueSources(tokensToExpression(tokens));
    const changed = () => {
      this.m_runtime.recordVariableChange(line, ref.path, method, tokensToExpression(tokens), before, this.readLValue(ref), sources);
      return true;
    };
    ...
    if (method === 'rotateBy') {
      ...
      if (isPoint(p)) {
        const axis = args[1];
        if (args.length < 2 || args.length > 3 || !isVector(axis)) throw runtimeError('FdPoint3d::rotateBy(angle, axis [, point])');
    ...
    if (method === 'normalize') {
      const v = slot.get(); if (!isVector(v)) throw runtimeError('normalize requires FdVector3d'); slot.set(v.normalize()); return changed();
    }
    if (method === 'mirror') {
      const v = slot.get(); const normal = args[0];
      if (!isVector(v) || args.length !== 1 || !isVector(normal)) throw runtimeError('mirror requires FdVector3d normal');
      slot.set(v.mirror(normal)); return changed();
    }
    if (method === 'set') {
      if (args.length !== 3) throw runtimeError('set requires x, y, z');
      const x = runtimeNumber(args[0]), y = runtimeNumber(args[1]), z = runtimeNumber(args[2]);
      const target = slot.get();
      if (isPoint(target)) { slot.set(new FdPoint3d(x, y, z)); return changed(); }
      if (isVector(target)) { slot.set(new FdVector3d(x, y, z)); return changed(); }
    }
    return false;
  }
  ...
  private executeFreeCall(tokens: readonly Token[], line: number): boolean {
    ...
    if (name === 'get_fln_size' || name === 'get_fln_thick' || name === 'get_fln_diam' || name === 'get_ldist' || name === 'get_ext_diam') {
      if (argGroups.length >= 2) {
        ...
        req.currentValue = req.defaultValue;
        this.m_runtime.recordParameterRequest(req);
        if (this.m_runtime.m_parameters.has(key)) this.writeLValue(dest, this.parameterTextToValue(this.m_runtime.m_parameters.get(key)!, current));
      }
      return true;
    }
    ...
    if (name === 'lineSegToLineSegInt' || name === 'lineToLineInt') {
      ...
      if (hit) {
        ...
        this.m_runtime.recordVariableChange(line, outRef.path, name, tokensToExpression(tokens), before, this.readLValue(outRef), sources);
      }

      const call = emptyApiCall();
      call.line = line;
      call.parentApiIndex = this.m_parentApiStack.length === 0 ? -1 : this.m_parentApiStack[this.m_parentApiStack.length - 1];
      call.name = name;
      call.arguments = [runtimeDeepCopy(a0), runtimeDeepCopy(a1), runtimeDeepCopy(b0), runtimeDeepCopy(b1), runtimeDeepCopy(this.readLValue(outRef))];
      for (const group of argGroups) call.argumentExpressions.push(tokensToExpression(group));
      call.display = name + '(';
      for (let i = 0; i < call.argumentExpressions.length; ++i) { if (i) call.display += ', '; call.display += call.argumentExpressions[i]; }
      call.display += ');';
    ...
    for (let i = 0; i < argGroups.length; ++i) {
      try {
        ...
        this.m_runtime.addDiagnostic(line, `cannot evaluate argument ${i + 1}`
          + ' of ' + name + ' (' + tokensToExpression(argGroups[i]) + '): ' + stdException(e).message);
        args.push(undefined);
    ...
    for (const group of argGroups)
      call.argumentExpressions.push(tokensToExpression(group));
    ...
    if (fn) {
      ...
      if (!hasUnresolvedArgument)
        this.executeUserFunction(fn, args, argGroups, functionIndex);
      return true;
    }
    ...
    if (!apiSignatureMetadataForCall(call)
  ...
  private functionParameters(fn: Statement): Token[][] {
    ...
    let lp = sig.length, rp = sig.length;
    let depth = 0;
    for (let i = 0; i < sig.length; ++i) {
      if (isSymbol(sig[i], '(')) { if (depth++ === 0) lp = i; }
      else if (isSymbol(sig[i], ')')) { if (--depth === 0) { rp = i; break; } }
    }
    if (lp >= rp || rp > sig.length) return [];
    let result = splitTopLevel(sliceTokens(sig, lp + 1, rp), ',');
    if (result.length === 1 && result[0].length === 0) result = [];
    else if (result.length === 1 && result[0].length === 1 && isIdentifier(result[0][0], 'void'))
      result = [];
    return result;
  }
  ...
  private bindFunctionArguments(fn: Statement, args: readonly RuntimeValue[],
    traces: readonly RuntimeArgumentTrace[]): void {
    const params = this.functionParameters(fn);
    for (let i = 0; i < params.length; ++i) {
      ...
      if (i < args.length) {
        ...
        if (eq < params[i].length) {
          try { value = runtimeDeepCopy(this.evaluate(sliceTokens(params[i], eq + 1, params[i].length))); }
          catch (e) { stdException(e); value = undefined; }
        }
      }
      const trace = i < traces.length ? traces[i] : null;
      const eq = this.parameterDefaultPos(params[i]);
      const expression = trace ? trace.expression
  ...
  private static lineIntersection(p1: FdPoint3d, p2: FdPoint3d,
    q1: FdPoint3d, q2: FdPoint3d,
    segments: boolean, out: Ref<FdPoint3d>): boolean {
    const u = p2.sub(p1);
    const v = q2.sub(q1);
    const w = p1.sub(q1);
  ...
  private executeUserFunction(fn: Statement, args: readonly RuntimeValue[],
    argumentTokens: readonly Token[][], parentApiIndex: number): void {
    if (this.m_functionCallDepth >= 64)
      throw runtimeError('C++ function call depth exceeded 64');
    ...
    interface ReferenceOutput { index: number; value: RuntimeValue; sources: RuntimeValueSource[] }
    ...
    for (let i = 0; i < params.length && i < argumentTokens.length; ++i) {
      ...
      outputs.push({ index: i, value: this.m_runtime.lookupValue(name), sources: this.m_runtime.captureValueSources(name) });
    }
    ...
    const callLine = (parentApiIndex >= 0 && parentApiIndex < this.m_runtime.m_apiCalls.length)
      ? this.m_runtime.m_apiCalls[parentApiIndex].line : fn.startLine;
    for (const output of outputs) {
      try {
        ...
        const after = this.readLValue(dest);
        if (runtimeValueToCompactString(before) !== runtimeValueToCompactString(after))
          this.m_runtime.recordVariableChange(callLine, dest.path, 'reference write-back', fn.functionName, before, after, output.sources);
      } catch (e) {
        this.m_runtime.addDiagnostic(callLine, 'cannot write back reference parameter of ' + fn.functionName + ': ' + stdException(e).message);
      }
    }
  }
    ...
    if (isIdentifier(tokens[0], 'typedef')) {
      ...
      if (parseRuntimeType(tokens, pos, base) && pos.value < tokens.length) {
        ...
        if (pos.value + 2 < tokens.length && isSymbol(tokens[pos.value], '[') && isSymbol(tokens[pos.value + 2], ']')) {
          extent = Math.trunc(tokens[pos.value + 1].number); pos.value += 3;
        }
        if (alias && sdkCanonicalType(base.value) === alias.baseType && extent === alias.arrayExtent && pos.value === tokens.length) return;
      }
  ...
  private initializeFunctionParameters(fn: Statement): void {
    ...
    for (const param of params) {
      ...
      try { this.executeDeclaration(param, fn.startLine, true); }
      catch (e) {
        stdException(e);
...
const kParameterQualifiers: readonly string[] = [
  'const', 'volatile', 'signed', 'unsigned', 'struct', 'class', 'typename',
];
...
export class GeometryRuntime {
  ...
  evaluateNumericExpression(expression: string): number {
    ...
    for (const [name, value] of this.m_values)
      snapshot.m_values.set(name, runtimeDeepCopy(value));
  ...
  executeUpToLine(code: string, maxLine: number): RuntimeResult {
    ...
    // runtime (for example SEGNUM) while keeping macro evaluation deterministic.
    {
      for (const lineText of code.split('\n')) {
        ...
        while (nameEnd < definition.length
          && (isalnum(definition[nameEnd]) || definition[nameEnd] === '_'))
          ++nameEnd;
    ...
    const appendValue = (name: string, value: RuntimeValue, depth: number): void => {
      ...
      result.variables.push({ name, value: runtimeDeepCopy(value), lastChangedLine: changed === undefined ? 0 : changed });
      if (depth > 4) return;
      if (!isArray(value)) return;
      const array = value;
      const limit = Math.min(array.elements.length, 64);
      for (let i = 0; i < limit; ++i)
        appendValue(`${name}[${i}]`, array.elements[i], depth + 1);
    };
    ...
    for (let i = 0; i < tokens.length; ++i) {
      ...
      while (p < tokens.length) {
        if (isSymbol(tokens[p], '[')) {
          ...
          const pureIndex = indexTokens.every((t) =>
            t.kind === TokKind.Number || t.kind === TokKind.Identifier
            || (t.kind === TokKind.Symbol && (t.text === '+' || t.text === '-'
      ...
      sources.push({ name: path, value: runtimeDeepCopy(value), variableId: id === undefined ? -1 : id, historyEnd: this.m_variableChanges.length });
    }
    return sources;
  }
  ...
  captureArgumentTrace(expression: string, value: RuntimeValue, depth = 0): RuntimeArgumentTrace {
    ...
    for (let i = 0; i < array.elements.length; ++i) {
      const elementExpression = initializer
        ? (i < parts.length ? tokensToExpression(parts[i]) : '')
  ...
  /** @internal */
  setVariable(name: string, value: RuntimeValue, userVariable = true, line = 0, operation = '', expression = '', inputTrace: RuntimeArgumentTrace | null = null): void {
    ...
    const trace: RuntimeArgumentTrace = inputTrace ? inputTrace
      : line > 0 ? this.captureArgumentTrace(expression, value) : { expression: '', sources: [], elements: [] };
    if (newLifetime)
      this.m_variableIds.set(name, this.m_nextVariableId++);
    if (userVariable && !existed) this.m_userVariableOrder.push(name);
    this.m_values.set(name, runtimeDeepCopy(value));
    if (line > 0) {
      this.recordVariableChange(line, name, operation === '' ? 'set' : operation, expression, before, this.m_values.get(name), trace.sources);
      ...
      const addChildren = (base: string, v: RuntimeValue, input: RuntimeArgumentTrace): void => {
        ...
        for (let i = 0; i < elements.length; ++i) {
          const child = `${base}[${i}]`;
          const elementTrace: RuntimeArgumentTrace = i < input.elements.length ? input.elements[i] : { expression: '', sources: [], elements: [] };
          this.recordVariableChange(line, child, 'declare', elementTrace.expression, undefined, elements[i], elementTrace.sources);
          addChildren(child, elements[i], elementTrace);
        }
      };
      if (!existed || operation === 'declare' || operation === 'bind') addChildren(name, this.m_values.get(name), trace);
    }
  }
  ...
  recordApiCall(call: RuntimeApiCall): number {
    ...
    for (let i = 0; i < call.arguments.length; ++i)
      call.argumentTraces.push(this.captureArgumentTrace(
        i < call.argumentExpressions.length ? call.argumentExpressions[i] : '', call.arguments[i]));
  ...
  recordVariableChange(
    line: number, name: string, operation: string, expression: string,
    before: RuntimeValue, after: RuntimeValue, sources: RuntimeValueSource[] = []): void {
    if (line <= 0 || name === '') return;
    const cut = name.search(/[[.]/);
    const root = cut === -1 ? name : name.slice(0, cut);
    const id = this.m_variableIds.get(root);
    this.m_variableChanges.push({
      line, name, operation, expression, before: runtimeDeepCopy(before), after: runtimeDeepCopy(after),
      variableId: id === undefined ? -1 : id, sources,
    });
  ...
  recordParameterRequest(request: RuntimeParameterRequest): void {
    const it = this.m_parameterRequests.findIndex((r) =>
      r.name === request.name && r.sourceFunction === request.sourceFunction && r.variableName === request.variableName);
...
export function runtimeSourceHistory(result: RuntimeResult, source: RuntimeValueSource): number[] {
  ...
  const containsPath = (parent: string, child: string) =>
    child.length > parent.length && child.startsWith(parent)
  ...
  for (let i = 0; i < end; ++i) {
    ...
    if ((change.operation === 'declare' || change.operation === 'bind')
      && isArray(change.after) && containsPath(change.name, source.name)) continue;
