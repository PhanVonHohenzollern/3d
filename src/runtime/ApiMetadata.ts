// Port of runtime/ApiMetadata.{h,cpp}: the native SDK signature registry and
// overload resolution. User-defined project functions are intentionally not
// stored here; GeometryRuntime discovers those directly from C++ source.
...
function typeCompatibilityScore(value: RuntimeValue, formalType: string): number {
  ...
  // Unknown/unset values should not prevent metadata lookup.
  return 0;
}
...
function signatureScore(sig: ApiSignatureMetadata, call: RuntimeApiCall): number {
  ...
  // Prefer the overload that needs the fewest omitted defaults.
  score -= sig.parameters.length - argc;
  return score;
}

/** Complete native/API metadata registry generated from the SDK interface headers. */
export function allNativeApiSignatures(): readonly ApiSignatureMetadata[] {
  return kNativeApiSignatures;
}

/**
 * Finds the best overload for a runtime call, using name, argument count,
 * default arguments and the runtime argument types when overloads share the
 * same arity. The first of equally scored overloads wins.
 */
...
/** Compatibility helper used by API Trace. */
