...
export function ParameterPanel(props: ParameterPanelProps) {
  const { fields, pagination, resetToSource } = useParameterPanel(props);

  const { containerRef: paginationRef, controls, columns: pageColumns, items: pageItems } = pagination;
    ...
    <div className="@container/parameters flex h-full min-h-0 flex-col bg-window">
      ...
      <div ref={paginationRef} className="flex min-h-0 flex-1 flex-col">
        {fields.length === 0 ? (
          ...
          <div
            className="grid min-h-0 flex-1 content-start gap-x-3 gap-y-2 overflow-hidden p-1.5"
            style={{ gridTemplateColumns: `repeat(${pageColumns}, minmax(0, 1fr))` }}
          >
            {pageItems.map((field) => (
